                        ...
                        <div class="col-md-4">
                            <div class="form-group">
                                ...
                                <select name="name" id="" class="form-control" placeholder="" required>
                                    ...
                                    <?php
                                    $sqlAsset = 'SELECT asset_in.Id, asset.AssetId, AssetName, Brand, Description, Price FROM asset JOIN asset_in ON asset_in.AssetId = asset.AssetId LEFT JOIN asset_assign ON asset_in.Id = asset_assign.AssetInId WHERE asset_assign.Id IS NULL';
                                    $resultAsset = $conn->query($sqlAsset);
                                    if($resultAsset->num_rows > 0) {
                                        while($rowAsset = $resultAsset->fetch_assoc()) {
                                            echo '<option value="'.$rowAsset['Id'].'">'.$rowAsset['AssetName']." - ".$rowAsset['Brand']." - ".$rowAsset['Description']." - ".$rowAsset['Price'].'</option>';
                                        }
                                    }
                                    ?>
                                </select>
